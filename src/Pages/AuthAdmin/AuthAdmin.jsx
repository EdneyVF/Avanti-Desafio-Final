...
import ServiceAdminAuth from "../../service/ServiceAdminAuth";
import "./Styles.css";
...
function AuthAdmin() {
  ...
  useEffect(() => {
    document.body.classList.add('AuthAdmin');
  
    return function cleanup() {
      document.body.classList.remove('AuthAdmin');
    };
  }, []);

  const handleLogin = async () => {
    try {
      const token = await ServiceAdminAuth.login(email, password);
  ...
  const handleSignup = async () => {
    try {
      await ServiceAdminAuth.signup(email, password);
    } catch (error) {
